...
4. ~~**The offline "erst online laden" appears twice** on a tap (the row's marker and the reply line under the list). Both are the wording the handoff asked for; drop the row marker if it feels doubled.~~ → fixed in 0025 (one toast per tap: a listed region already carries the marker, so the tap only sets the line for an unlisted one)
