...
5. **`outside` carries a backbone key from `backboneSearch`**: three GBIF calls plus per-key lookups, 1–2 s on top of the scan, only on outside answers at ≥ 0.7. Nothing is created, but "Das ist es" on it goes through `taxon.ensure`, which does create the row and kicks the content job. Same as a typed search today. → accepted in 0025: outside answers create backbone rows; a find outside the set is still a find, the row is what the diary and the trailing atlas block show
