...
| A4 | ~~The function fetches `iad1` from `fra1` on every uncached view; a scraper with the capability URL and no worker costs one Blob read per hit. Same exposure as the disk had, priced now~~ → fixed in 0025 (sounds are CC content and now `cache-control: public, max-age=31536000, immutable`; photos stay `private`, the exposure is unchanged there) | low |
...
| B2 | **A region job through `waitUntil` needs ~2 min for the set plus the content run** (111 s for Mainz-Bingen's set, then 75 min of content). `maxDuration` 300 covers the set; the content run dies at 300 s and the hourly sweep finishes it in 4-minute slices: ≈ 20 hours for a new region. Fine for one user and no new regions from the UI (CLAUDE.md); a queue is the honest fix when regions open → accepted in 0025: regions are filled locally and dumped (CLAUDE.md rule); the honest fix is a queue when regions open, not now | medium, by design |
...
| B4 | **Two instances, two kicks.** The `globalThis` maps dedupe within a warm instance only; two requests for the same key on two instances both run the content job. The job serialises its writes (findings 0008 A11); double work, no corruption → accepted in 0025: regions are filled locally and dumped (CLAUDE.md rule); the double kick exists only for a region nobody opens | low |
