...
| B3 | ~~The tile proxy has no rate limit: a stranger could pull OSM through the app's origin~~ → fixed in 0025 (a per-address token bucket, 600 tiles/min, 429 beyond it, plus a same-origin check) | Nine tiles per region at one zoom; Caddy could add a `rate_limit` later. Not before there is a second user |
