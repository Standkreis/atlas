...
5. **P4 and the tab bar.** The handoff says "the tab bar stays as the second way out", but `SpeciesPage.tsx:105` hides it (`[&~nav]:hidden`, from the mock). Now the back button is the *only* way out of a species page. Either the bar comes back on the species page (one class) or the sentence goes. → accepted in 0025: the species page is a leaf reached by a tap, its Back goes to the tab that opened it; a tab bar on a leaf would offer four exits from a page that has one
6. ~~**Amber.** `#c4620f` is one step towards orange, still under AA for 15 px bold text on white (4.1 : 1; the old one was 3.9). Reaching 4.5 : 1 means `#b45309` (5.0), which is darker, not more saturated.~~ → fixed in 0025 (new token `--color-amber-deep`, `#a94c08` light / `#f0a030` dark, passes 4.5:1 on white, paper and the banner; brand amber kept for fills)
...
4. **T1 · the drawer has no URL.** Back does not close it (Escape, drag, tap outside and Schließen do); a refresh on the diary loses it. `pushState` to `/sighting/<id>` would make the App Router render the route. Acceptable for a transient sheet; say so if not. → accepted in 0025: the sighting drawer is modal by design, it opens over the diary and closes back to it; a URL would make Back leave the diary
...
5. ~~**Avatar on the static export**: `photoSrc` prefixes `NEXT_PUBLIC_API_URL`, as for sighting photos. The worker does not precache it; it is fetched when the profile opens, so offline the circle falls back to the browser's cache or the alt text. The profile is not part of the walk.~~ → fixed in 0025 (the worker already caches it under the `/api/photo/` image rule once viewed online, same as species images; only `splash.jpg` needed adding)
