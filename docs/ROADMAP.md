...
The owner's weekly plan limits ran out mid-0028. State for the next session, verified 2026-09-08:

| What | State |
| --- | --- |
| `main` | clean tree, **18 commits unpushed** (`67391fc` ladder photo on top … `b6199fa` run r1). Live is still build `mtrahjxp` (a9ba986). **Push only when the owner says "push"** |
| Check | `npm run check` green at `b6199fa`: 111 tests, export build |
| Worktrees | none; all track branches merged (grouping, m9b-grill, progress, prose-2, prose-a, prose-b, sweep-a/b/c). Branches kept, deletable |
| Dev DB | Mainz-Bingen refetched with GloBI studies (929 taxa, 105 887 of 123 558 edges `prose=false`); **five taxa hold `Taxon.prose`** (2431776 Feuersalamander, 8168319 Fliegenpilz, 2475532 Eisvogel, 1929697 Kleiner Feuerfalter, 5743122 Hirschkäfer). Kyoto, Schagen, Südwestpfalz are **not** refetched (F2 never fires there until `content --region <name> --force`, ≈ 3.5 h per 900 taxa) |
| `app/etl/prose/runs/r1/` | git-ignored; 18 prompts + 36 answers of the five, regenerable |
| Neon | nothing of 0028 there yet; the deploy adds the columns, the dump (ETL README §🚀 option 2) carries `Taxon.prose` and `Interaction.studies/real/prose` |
| Console key | `ANTHROPIC_API_KEY` untouched in `app/.env.local`; no script of 0026–0028 calls the API any more (CLAUDE.md "Never") |

Next, in order:

| # | Step | Who |
| --- | --- | --- |
| 1 | Owner reviews the five texts ([findings 0028 §🔎](handoffs/0028-prose-findings.md)) in the Simulator: `cd app && npm run build && npx next start -p 3002`, species 2431776 · 8168319 · 2475532 · 1929697 · 5743122 | owner |
| 2 | Decide: whole region (≈ 1 200 Sonnet subagents ≈ 2 h 15 min, ≈ 28 M plan tokens) · a slice · or first the two data gaps the read found (partner names without a German label, maxima written as typical: "wird 28 cm lang") | owner |
| 3 | "push" → deploy (migration `20260910000000_prose`) → verify `/api/health` build id → the ladder layout (67391fc) checked with one scan | owner + agent |
| 4 | The region run: `npm run etl -- prose --region mainz-bingen --run r2` → draft agents (five prompts each, Sonnet, drafts only) → the command again → audit agents → `prose --load --run r2` → ten Ökologie paragraphs hand-read into the findings → dump to Neon | agent |
| 5 | Still open from 0025: `facts --force` and `sounds --region` on Neon, phone checklist P1–P5 | owner |
...
| 0028 | The prose ([findings 0028](handoffs/0028-prose-findings.md), tracks [A](handoffs/0028-prose-findings-a.md) · [B](handoffs/0028-prose-findings-b.md)): `Taxon.prose` and `Interaction.studies/real/prose` by migration, GloBI edges refetched with their studies and pruned F1/F2 for prose (Mainz-Bingen 105 887 of 123 558 edges out), the `prose` ETL step with the `files` driver (prompts and answers as files for Sonnet subagents on the plan, the `api` seam off), `<Prose>` after the Steckbrief tiles and the Ökologie paragraph under its tile with the ⓘ sheet of cited lines. 111 tests. **Five species written on the plan** (76 supported · 2 partial · 0 unsupported by the judge, ≈ 9 min); the region is ≈ 1 200 subagents ≈ 2 h 15 min once the owner has reviewed the five |
