...
| `insect` | Wirbellose | Invertebrates | Existing 3,660-record non-chordate animal bucket; detailed Insekten is a separate choice. Help explicitly scopes this legacy branch: the ascidian remains under Fish & other chordates, so this is not an exhaustive scientific set of all invertebrates |
...
| `plant.other` | Weitere Pflanzen / Other plants | Positive known taxonomic remainder: currently six Lycopodiopsida245 and 13 Charophyta/Chlorophyta records. This is not negative growth-form evidence; a separately proven growth-form membership may overlap. Remain accessible under all plants |
| `plant.unknown` | Pflanzengruppe noch unklar / Plant group not yet resolved | Plants with no supported primary browse shortcut. Current audit leaves 1,685 after its taxonomic partitions; no woody/herbaceous count is promised. A known grass/fern/etc record with an unresolved secondary growth-form axis does not enter this selectable bucket |
...
| `animal.other` | Weitere Wirbellose / Other invertebrates | Source-backed remaining non-chordate animals outside approved shortcuts; 82 known residuals conditional on the candidate class union. If that mapping is deferred, keep those 82 known residuals and 39 unresolved candidates separate, with all 121 accessible in the broad view |
...
“Processed all 6,874” must report how many were classified, unknown, conflicted and failed per axis. A failure cannot be converted into biological nonmembership. A record can have a proven taxonomic shortcut and an unresolved growth-form axis; it stays selectable by its proven shortcut. The selectable `plant.unknown` bucket means no supported primary shortcut, not every missing secondary trait. Per-axis unresolved/conflict/failed outcomes remain separately counted in the audit and available in source detail; an additional selectable diagnostic facet is deferred. Unknown is epistemic; the plant residual is the explicitly keyed taxonomic remainder, not a claim of exclusion from every possible growth form.
...
For **Nach Artengruppe**, proposed primary priority is grass-family → fern/horsetail → moss/liverwort → known other plants → woody → herbaceous → unresolved. Animal primary buckets follow the hierarchy above; detailed insect groups live under insects. Known membership wins display priority over an unresolved secondary axis. Such a record stays out of the primary-unknown filter; source detail can still explain its unresolved secondary evidence. Bucket assignment never depends on which shortcut was selected. OR selection and counts deduplicate by the unchanged accepted Taxon ID. This priority is reviewable and versioned; it must not hide a conflict or inflate totals.
...
**Nested selection normalization.** Within a broad parent's subset, store an antichain of selection nodes: never an ancestor together with its descendants. Selecting a nested all-parent removes its selected descendants and inserts that parent, preserving selected sibling branches. Deselecting a child covered by an all-ancestor first expands that ancestor into every declared immediate child (including residual/unknown), recursively along the changed path, then removes the target subtree. Expand selection structure, not a snapshot of currently matching taxa. Deselecting a nested all-parent removes that subtree only. If nothing remains in the broad branch, set detailed none; keep its broad interest enabled. Selecting every declared child, including residual/unknown even if currently count0, may canonically collapse to the all-parent. New children under a version update follow the explicit all versus subset intent; never infer all merely because selected groups currently cover every taxon.

Example: broad `insect` refinement `{animal.insect, animal.arachnid}` selects all insects plus arachnids. Deselecting beetles yields `{insect.lepidoptera, insect.odonata, insect.other, insect.unknown, animal.arachnid}` under the proposed first insect subdivision. Coleoptera is excluded, all other insect branches and arachnids remain. Selecting all insects collapses those insect leaves back to `animal.insect`; clearing the insect subtree leaves only `animal.arachnid`. Clearing that last detailed branch yields none, never legacy empty-tiles=all. Tests must cover these transitions through URL reload and region change as well as direct clicking.
...
**1280/1440 px desktop, English.** Keep the centered 520 px Atlas; the same keyboard-accessible modal opens over it. With Berlin and only the Insecta detail selected, whole-year regional result count636; broad invertebrate progress denominator728. An optional insect disclosure contains butterflies & moths214, beetles135, dragonflies & damselflies25 and remaining other insects262. Subset selection can narrow these, while the all-insects parent remains reachable without visiting every child. Tab reaches separate disclosure/checkbox targets; Space toggles; Escape closes and restores the opener.

| Review state | Visible behavior and future assertion |
| --- | --- |
| 320/390 px, EN and DE | No horizontal scroll or clipped long labels; at least 44 px targets; count stays associated with label; modal content scrolls with a reachable sticky result action |
