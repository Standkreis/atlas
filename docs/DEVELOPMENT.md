...
Runtime maintenance removes expired codes, quota buckets and abandoned/retryable media. Large region and content tasks run via `npm run etl -- region <name>` and `npm run etl -- sweep` on the development machine. A newly logged outside-set species is saved immediately with GBIF names; rich content waits for the CLI sweep. The CLI holds a session advisory lock, not an hours-long SQL transaction. HTTP reads have timeouts. `refresh` bypasses the disk cache; other cached GETs expire after 30 days. Both map and ETL read the versioned observation window in `src/domain/observationWindow.ts`; advancing it requires a coordinated region refresh.
