...
export function SightingDetail({ id, mode, origin, onGone }: { id: string; mode: 'page' | 'drawer'; origin: string; onGone: () => void }) {
  ...
  const invalidate = () => Promise.all([qc.invalidateQueries({ queryKey: trpc.journal.pathKey() }), qc.invalidateQueries({ queryKey: trpc.identity.progress.queryKey() }), qc.invalidateQueries({ queryKey: trpc.identity.germanyProgress.queryKey() }), qc.invalidateQueries({ queryKey: trpc.sighting.photos.queryKey() }), qc.invalidateQueries({ queryKey: trpc.sighting.outsideVersioned.pathKey() }), qc.invalidateQueries({ queryKey: trpc.sighting.fill.queryKey({ id }) })])
