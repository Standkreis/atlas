...
import { boundedScan, consume, dailyAllowance, lock } from './quotas'
...
async function photo(ownerId: string) {
  return db.asset.create({ data: { ownerId, origin: 'user', kind: 'image', url: '', sourceUrl: '', author: '', licence: '', byteSize: 100 } })
}
let taxonId: string
beforeEach(async () => {
  await db.quotaBucket.deleteMany()
...
describe('durable expensive-operation admission and deletion', () => {
  ...
  it('rejects the monthly spending ceiling independently of the daily allowance', async () => {
    ...
    await db.quotaBucket.create({ data: { key, value: 30000, expiresAt: new Date(Date.now() + 86400000) } })
