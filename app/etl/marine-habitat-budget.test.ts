...
it('charges real WoRMS HTTP attempts to ETL_BUDGET and preserves the first 50-name checkpoint', async () => {
  ...
  const names = Array.from({ length: 51 }, (_, index) => `Budget example${index}`)
  ...
  expect(batches[0]!.names).toHaveLength(50)
