...
import { buildReviewedGalleryPlan, type CatalogueTargetRow, type TargetCatalogueSnapshot } from './catalogue-import-plan'
...
describe('buildReviewedGalleryPlan', () => {
  it('reuses an explicitly bound identity and preserves the old lead before a new image', () => {
    ...
    const sourceNew = asset('source-new', 'source-taxon', { position: 1 })
    ...
    expect(result.assetMutations[0]?.after).toMatchObject({ id: 'source-new', taxonId: 'target-taxon' })
